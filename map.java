...
public class Map {
	...
	HashMap<PathSummary, Path> cachedPaths = new HashMap<PathSummary, Path>();
	
	HashMap<Unit, Vertex[]> buildings = new HashMap<Unit, Vertex[]>();
	...
	private class PathSummary {
		...
		public PathSummary(int startx, int starty, int endx, int endy) {
			
			start = getTriangle(startx, starty);
			end = getTriangle(endx, endy);
		}
		...
		public int hashCode() {
			...
			return start.vertices[0].x * start.vertices[0].y * end.vertices[0].x * end.vertices[0].y *
					start.vertices[1].x * start.vertices[1].y * end.vertices[1].x * end.vertices[1].y *
					start.vertices[2].x * start.vertices[2].y * end.vertices[2].x * end.vertices[2].y;
		}
		
		public boolean equals(Object obj) {
			if(!(obj instanceof PathSummary)) {
				return false;
			} else {
				PathSummary other = (PathSummary) obj;
	...
	private void removeVertex(Vertex v) {
		...
		if(trianglesForReconstitution.size() == 3) {
			
//			LinkedHashSet<Vertex> vertices = new LinkedHashSet<Vertex>();
//			
//			for(Triangle t : trianglesForReconstitution) {
//				for(int i = 0; i < 3; i++) {
//					vertices.add(t.vertices[i]);
//				}
//			}
//			
//			vertices.remove(v);
//			
//			Triangle newTri = new Triangle((Vertex[]) vertices.toArray());
//			
...
//	private boolean CheckAndFlip() {
//    	for(Triangle t : triangles) {
//    		CheckAndFlip(t);
//    		Vertex cc = circleCenter(t.vertices[0], t.vertices[1], t.vertices[2]);
//    		int radius = cc.distance(t.vertices[0]) - 2;
//    		for(Triangle tt : triangles) {
//    			
//    			if(tt.hasPointInCircle(cc, radius)) {
//    				if(tt != t) {
//    					if(t.share2points(tt)) {
//    						flip(t, tt);
//        					return false;
//    					}
//    				}
